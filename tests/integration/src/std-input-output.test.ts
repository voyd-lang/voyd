...
describe("integration: std input/output", () => {
  ...
  it("supports bare print and enum from the implicit prelude", async () => {
    ...
    await expect(
      host.run<number>("prelude_log_module_with_assertions_all"),
    ).resolves.toBe(1);
  ...
  it("covers read eof/error and output error paths", async () => {
    ...
    await expect(outputErrorHost.run<number>("io_output_error")).resolves.toBe(
      1,
    );
  });
});
