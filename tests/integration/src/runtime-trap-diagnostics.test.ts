...
describe("integration: runtime trap diagnostics", () => {
  ...
  it("does not require a custom panic import to instantiate the module", async () => {
    ...
    expect(
      imports.some(
        ({ module, name }) => module === "env" && name === "__voyd_panic_trap",
      ),
    ).toBe(false);
  });
  ...
  it("surfaces module/function/span diagnostics for pure wasm traps", async () => {
    const error = await expectRuntimeTrap(
      compiled.run({ entryName: "pure_trap" }),
    );
    ...
    expect(error.voyd.trap.span?.file).toContain(
      "runtime-trap-diagnostics.voyd",
    );
  ...
  it("preserves panic message text in runtime diagnostics", async () => {
    const error = await expectRuntimeTrap(
      compiled.run({ entryName: "panic_trap" }),
    );
