...
const createFixture = async (): Promise<{
  ...
  const root = await fs.mkdtemp(
    path.join(tmpdir(), "voyd-doc-generation-smoke-"),
  );
...
const createPackageRootFixture = async (): Promise<{
  ...
  const root = await fs.mkdtemp(
    path.join(tmpdir(), "voyd-doc-generation-pkg-root-"),
  );
  const srcRoot = path.join(root, "src");
...
const createNamedReExportPackageRootFixture = async (): Promise<{
  ...
  const root = await fs.mkdtemp(
    path.join(tmpdir(), "voyd-doc-generation-pkg-reexport-"),
  );
  const srcRoot = path.join(root, "src");
  const entryPath = path.join(srcRoot, "pkg.voyd");
  await fs.mkdir(srcRoot, { recursive: true });
...
describe("integration: sdk doc-generation", () => {
  it("generates HTML and JSON documentation outputs", async () => {
    ...
    try {
      ...
      const mainModule = parsed.modules.find(
        (module) => module.id === "src::main",
      );
      expect(mainModule).toBeDefined();
      const addOne = mainModule?.functions.find((fn) => fn.name === "add_one");
      expect(addOne).toBeDefined();
      const enumMacro = mainModule?.macros.find(
        (macro) => macro.name === "enum",
      );
      ...
      const valueDoc = addOne?.parameterDocs.find(
        (param) => param.name === "value",
      );
      expect(valueDoc?.documentation).toBe(" Input value.\n Keep newline.");
      const labeledFn = mainModule?.functions.find(
        (fn) => fn.name === "take_labeled",
      );
      expect(labeledFn?.signature).toContain("{ n: i32 }");
      const foldLike = mainModule?.functions.find(
        (fn) => fn.name === "fold_like",
      );
      ...
      const duplicateMethodFn = mainModule?.functions.find(
        (fn) => fn.name === "double",
      );
      expect(duplicateMethodFn).toBeUndefined();
      const hiddenMethodFn = mainModule?.functions.find(
        (fn) => fn.name === "hide",
      );
      ...
      const doubleMethod = counterImpl?.members.find(
        (member) => member.name === "double",
      );
      ...
      const hiddenMethod = counterImpl?.members.find(
        (member) => member.name === "hide",
      );
      expect(hiddenMethod).toBeUndefined();
      const hiddenImpl = mainModule?.impls.find((impl) =>
        impl.signature.includes("Hidden"),
      );
      expect(hiddenImpl).toBeUndefined();
      const externalImpl = mainModule?.impls.find((impl) =>
        impl.signature.includes("External"),
      );
      ...
      const counterObject = mainModule?.objects.find(
        (objectDecl) => objectDecl.name === "Counter",
      );
      expect(counterObject?.members.map((member) => member.name)).toEqual([
        "value",
      ]);
      const meterObject = mainModule?.objects.find(
        (objectDecl) => objectDecl.name === "Meter",
      );
      expect(meterObject?.signature).toBe("val Meter");
      const decodeEffect = mainModule?.effects.find(
        (effectDecl) => effectDecl.name === "Decode",
      );
      expect(decodeEffect?.documentation).toBe(" Decode effect docs.");
      const decodeNextOp = decodeEffect?.members.find(
        (member) => member.name === "decode_next",
      );
  ...
  it("only renders modules exported by pkg.voyd", async () => {
    ...
    try {
      ...
      expect(html.content).not.toContain(
        'id="module-src-pkg-visible-hidden-nested"',
      );
    } finally {
      await fs.rm(fixture.root, { recursive: true, force: true });
    }
