...
const perfIterations = Number.parseInt(
...
benchmarkDescribe("performance: vtrace compute-only benchmark", () => {
  ...
  mainIt(
    "renders the compute-only checksum deterministically without output effects",
    async () => {
  ...
  it("keeps the plain effect RNG probe deterministic", async () => {
    const result = await compiled.run<number>({
      entryName: "debug_effect_rand_plain",
    });
  ...
  it("keeps the ranged effect RNG probe deterministic", async () => {
    const result = await compiled.run<number>({
      entryName: "debug_effect_rand_range",
    });
  ...
  it("keeps the Vec3 ranged effect RNG probe deterministic", async () => {
    const result = await compiled.run<number>({
      entryName: "debug_effect_rand_vec3_sum",
    });
  ...
  it("keeps the empty-world checksum deterministic", async () => {
    const result = await compiled.run<number>({
      entryName: "debug_empty_world_checksum",
    });
