...
export const mapBinaryenType = (
  ...
  if (type.isTraitType()) {
    return buildObjectType(opts, voydBaseObject);
  }
  if (type.isFnType()) {
    return getClosureSuperType(opts.mod);
  }
  if (type.isSelfType()) {
    return buildObjectType(opts, voydBaseObject);
  }
  if (type.isTypeAlias()) {
    ...
    return mapBinaryenType(opts, typeAlias.type!);
  }
  if (type.isUnionType()) {
    return buildUnionType(opts, type);
  }
  if (type.isIntersectionType()) {
    return buildIntersectionType(opts, type);
  }
  if (type.isFixedArrayType()) {
    return buildFixedArrayType(opts, type);
  }
  if (type.isTupleType?.()) {
    return opts.mod.nop();
  }
...
export const buildFixedArrayType = (
  opts: MapBinTypeOpts,
  type: FixedArrayType
): TypeRef => {
  if (type.binaryenType) return type.binaryenType;

  type.binaryenType = gc.defineArrayType(
    opts.mod,
    mapBinaryenType(opts, type.elemType!),
    true
  );
