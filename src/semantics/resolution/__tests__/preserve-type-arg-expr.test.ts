...
import { Fn, Identifier, List, Parameter } from "../../../syntax-objects/index.js";
...
describe("Preserve type-arg expressions on specialization", () => {
  test("resolveFn stores original type arg expr on appliedTypeArgs", () => {
    ...
    const fn = new Fn({
      ...
      parameters: [new Parameter({ name: Identifier.from("arg"), typeExpr: arrayT })],
      typeParameters: [T],
      parent: mod,
    });
    ...
    const applied = inst.appliedTypeArgs?.[0] as any;
    expect(applied?.isTypeAlias?.()).toBe(true);
    expect(applied?.typeExpr?.isType?.()).toBe(true);
    expect(applied?.type?.isObjectType?.()).toBe(true);
    expect(applied?.type?.name?.value).toBe("String");
  });

  test("resolveTrait stores original type arg expr on appliedTypeArgs", () => {
    const T = Identifier.from("T");
    const trait = new TraitType({ name: Identifier.from("Iterable"), methods: [], typeParameters: [T] });
    // Register 'String' in a root module so the cloned trait instance can resolve
    const tmod = new RootModule({});
    tmod.registerEntity(new ObjectType({ name: Identifier.from("String"), value: [] }));
    trait.parent = tmod;
