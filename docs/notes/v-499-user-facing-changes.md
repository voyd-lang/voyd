...
- Host transport implementations use
  `@compiler_impl(id: ..., version: ...)` on an implementation of the
  compiler-contract `HostTransportProvider<Reader, Writer>` trait. The compiler
  derives the concrete lifecycle methods and reader/writer types from that
  implementation. The former object-level `@host_transport` annotation and
  MessagePack-specific transport function contracts have been removed.
