...
const isUsePathExpr = (expr: Expr | undefined): expr is Expr => {
  ...
  return (
    ...
    expr.calls(":") ||
    expr.calls("as") ||
    expr.callsInternal("object_literal")
  );
};
...
export const classifyTopLevelDecl = (
  ...
  if (keyword && MACRO_DECL_KEYWORDS.has(keyword)) {
    const signature = form.at(offset + 1);
    const potentialName = isForm(signature) ? signature.at(0) : undefined;
  ...
  if (visibility === "pub" && keyword && PUB_DECL_KEYWORDS.has(keyword)) {
    return { kind: "other" };
  }
