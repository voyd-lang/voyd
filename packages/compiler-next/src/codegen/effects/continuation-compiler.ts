...
export const createContinuationExpressionCompiler = ({
  ...
  const compileExpr: ExpressionCompiler = ({
    ...
    const resumeTarget =
      resumeActive && exprContainsTarget(exprId, targetExprId, ctx)
        ? targetExprId
        : undefined;
...
): boolean => (cfg.sitesByExpr.get(exprId)?.size ?? 0) > 0;
  ...
  expr.statements.forEach((stmtId) => {
    ...
    const shouldRun =
      sites.size === 0
        ...
        : ctx.mod.i32.or(
            started(),
            activeSiteInSet({ sites, activeSiteOrder, ctx })
          );
    statements.push(
      ctx.mod.if(
        shouldRun,
...
const compileGroupedContinuationIfExpr = ({
  ...
  if (typeof expr.defaultBranch === "number") {
    ...
    if (defaultSites.size > 0) {
      ...
      fallback = {
        ...
        usedReturnCall:
          defaultExpr.usedReturnCall && fallback.usedReturnCall,
      };
    }
  }
  ...
  for (let index = expr.branches.length - 1; index >= 0; index -= 1) {
    ...
    const valueSites =
      cfg.sitesByExpr.get(branch.value) ?? new Set<number>();
...
const compileGroupedContinuationMatchExpr = ({
  ...
  for (let index = expr.arms.length - 1; index >= 0; index -= 1) {
    ...
    const valueSites =
      cfg.sitesByExpr.get(arm.value) ?? new Set<number>();
    if (valueSites.size === 0) continue;
    const guardSites =
