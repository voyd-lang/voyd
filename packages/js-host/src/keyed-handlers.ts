...
export const parseHandlerKey = (key: string): ParsedHandlerKey => {
  ...
  if (maybeSignatureHash) {
    return { effectId, opName, signatureHash: maybeSignatureHash };
  }
...
export const registerHandlersByKey = ({
  host,
  handlers,
}: {
  host: KeyedHandlerHost;
